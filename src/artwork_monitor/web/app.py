...
def create_app(
    *, dependencies: WebDependencies | None = None, database_path: Path | None = None
):
    ...
    try:
        ...
        if error.name == "flask":
            raise RuntimeError(
                "Flask is optional; install artwork-transportation-monitor[web]"
            ) from error
        raise
    ...
    if dependencies is None:
        path = (
            database_path
            or Path(mkdtemp(prefix="artwork-monitor-web-")) / "web.sqlite3"
        )
    ...
    def transport_page():
        return render_template(
            "transport.html", state=dependencies.transport_workflow.state.value
        )
    ...
    def transport_start():
        ...
        try:
            dependencies.transport_workflow.start(
                TransportSession(session_id.strip(), started_at)
            )
        except RuntimeError as error:
            return jsonify({"error": str(error)}), 409
        payload = {
            "state": dependencies.transport_workflow.state.value,
            ...
            if cycle.gps_fix is not None and cycle.gps_fix.is_available:
                realtime.publish(
                    "gps_update",
                    {
                        ...
                        "timestamp": timestamp(cycle.gps_fix.timestamp),
                        "latitude": cycle.gps_fix.latitude,
                        "longitude": cycle.gps_fix.longitude,
                    },
                )
            for kind, violations in (
                ("immediate", cycle.immediate_violations),
                ("prolonged", cycle.prolonged_violations),
            ):
    ...
    def transport_stop():
        data = request.get_json(silent=True) or {}
        try:
            completed = dependencies.transport_workflow.complete(
                _parse_timestamp(data.get("ended_at"))
            )
        except ValueError as error:
            return jsonify({"error": str(error)}), 400
        except RuntimeError as error:
            return jsonify({"error": str(error)}), 409
        payload = {
            "state": TransportSessionState.COMPLETED.value,
            "session_id": completed.stored_session.session.session_id,
        }
        ...
        realtime.publish(
            ...
            {
                "session_id": completed.stored_session.session.session_id,
                "report_url": f"/report?session_id={completed.stored_session.session.session_id}",
            },
        )
        return jsonify(payload)
    ...
    def check_page():
        return render_template(
            "check.html",
            checking=dependencies.artwork_workflow.checking,
            artworks=_artworks(dependencies),
        )
    ...
    def check_start():
        ...
        realtime.publish(
            ...
            {"checking": True, "artworks": _artworks(dependencies)},
        )
    ...
    def check_step():
        ...
        if transition is not None:
            realtime.publish(
                ...
                {"transition": transition, "artworks": _artworks(dependencies)},
            )
    ...
    def check_stop():
        ...
        realtime.publish(
            "artwork_check_stopped",
            {"checking": False, "artworks": _artworks(dependencies)},
        )
    ...
    def report_page():
        ...
        return render_template(
            "report.html",
            session_ids=dependencies.session_repository.list_session_ids(),
            report=report,
        )
    ...
    def gps_page():
        return render_template(
            "gps.html", session_ids=dependencies.session_repository.list_session_ids()
        )
    ...
    def gps_history():
        ...
        points = [
            {
                "timestamp": timestamp(record.gps_fix.timestamp),
                "latitude": record.gps_fix.latitude,
                "longitude": record.gps_fix.longitude,
            }
...
def _parse_timestamp(value: Any) -> datetime:
    ...
    return (
        ...
        else timestamp.replace(tzinfo=HONG_KONG)
    )
    ...
    return [
        {
            ...
            "lot": state.identity.lot,
            "status": state.status.value,
            "time_in": timestamp(state.time_in) if state.time_in else None,
            "time_out": timestamp(state.time_out) if state.time_out else None,
        }
...
def _transition(transition):
    if transition is None:
        return None
    return {
        "label_index": transition.label_index,
        "status": transition.status.value,
        "occurred_at": timestamp(transition.occurred_at),
    }
...
def _report_or_none(dependencies: WebDependencies, session_id: str | None, abort):
    if session_id is None:
        return None
    try:
        return dependencies.report_generator.generate_from_repository(
            dependencies.session_repository, session_id
        )
    except KeyError:
        abort(404)
...
def _report_json(report) -> dict[str, Any]:
    return {
        ...
        "temperature": {
            "minimum": report.temperature.minimum,
            "maximum": report.temperature.maximum,
            "mean": report.temperature.mean,
        },
        "humidity": {
            "minimum": report.humidity.minimum,
            "maximum": report.humidity.maximum,
            "mean": report.humidity.mean,
        },
        "light": {
            "minimum": report.light.minimum,
            "maximum": report.light.maximum,
            "mean": report.light.mean,
        },
...
def _cycle_payload(session_id: str, cycle) -> dict[str, Any]:
    return {
        ...
        "reading": {
            "timestamp": timestamp(cycle.reading.timestamp),
            "temperature_c": cycle.reading.temperature_c,
            "humidity_percent_rh": cycle.reading.humidity_percent_rh,
            "light_lux": cycle.reading.light_lux,
            "gravity_deviation_g": cycle.reading.gravity_deviation_g,
        },
        "immediate_conditions": [
            violation.condition.value for violation in cycle.immediate_violations
        ],
        "prolonged_conditions": [
            violation.condition.value for violation in cycle.prolonged_violations
        ],
    }
