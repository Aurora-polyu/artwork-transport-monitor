...
from artwork_monitor.application import MonitoringService, SessionReportGenerator, TransportSessionWorkflow
from artwork_monitor.domain import GPSFix, GPSFixStatus, HONG_KONG, InferenceResult, SensorReading
...
class RealtimeWebTests(unittest.TestCase):
    ...
    def test_factory_initializes_socketio_without_starting_work(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            self.assertFalse(app.extensions["artwork_monitor_dependencies"].artwork_workflow.checking)
    ...
    def test_explicit_transport_actions_emit_ordered_session_scoped_events(self) -> None:
        with TemporaryDirectory() as temporary:
            app = self._app(Path(temporary))
            ...
            self.assertEqual(http.post("/transport/start", json={"session_id": "rt-1", "started_at": started_at}).status_code, 201)
            self.assertEqual(http.post("/transport/step", json={"monotonic_seconds": 0}).status_code, 200)
            self.assertEqual(http.post("/transport/stop", json={"ended_at": started_at}).status_code, 200)
            events = socket.get_received()

            self.assertEqual([event["name"] for event in events], ["transport_started", "transport_cycle", "gps_update", "transport_completed", "report_ready"])
            self.assertTrue(all(event["args"][0]["session_id"] == "rt-1" for event in events))
    ...
    def test_violation_event_contains_existing_condition_data(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            reading = SensorReading(timestamp=datetime(2026, 8, 9, 12, 0, tzinfo=HONG_KONG), temperature_c=27.1)
            ...
            http.post("/transport/start", json={"session_id": "hot", "started_at": started_at})
            http.post("/transport/step", json={"monotonic_seconds": 0})

            events = socket.get_received()
            violation = next(event["args"][0] for event in events if event["name"] == "violation")
    ...
    def test_artwork_transition_emits_status_change_only_after_legacy_cadence(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            self.assertEqual([event["name"] for event in events], ["artwork_check_started", "artwork_status_changed"])
    ...
    def test_separate_apps_do_not_share_realtime_state(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            self.assertEqual([event["name"] for event in first_socket.get_received()], ["artwork_check_started"])
