...
class RecordingAlarmOutput:
    ...
    def reset(self) -> None:
        raise AssertionError(
            "MonitoringService must not globally reset an alarm output"
        )
...
class MonitoringServiceTests(unittest.TestCase):
    ...
    def test_temperature_immediate_and_prolonged_conditions_remain_distinct(
        self,
    ) -> None:
        service = MonitoringService(
            SequenceSensorSource(tuple(reading(temperature_c=27.1) for _ in range(4)))
        )
        service.start()
        ...
        self.assertTrue(
            all(
                ...
                in self._conditions(result.immediate_violations)
        ...
        self.assertEqual(
            self._conditions(results[2].prolonged_violations),
            (Condition.TEMPERATURE_HIGH,),
        )
    ...
    def test_multiple_immediate_violations_are_preserved(self) -> None:
        result = run_simulated_session(
            scenarios.multiple_environmental_violations(), (0.0,)
        )[0]
        self.assertEqual(
    ...
    def test_missing_measurement_resets_prolonged_continuity(self) -> None:
        ...
        self.assertEqual(
            self._conditions(results[3].prolonged_violations),
            (Condition.TEMPERATURE_HIGH,),
        )

    def test_darkness_does_not_suppress_acceleration_or_gravity_deviation(self) -> None:
        result = run_simulated_session(
            scenarios.dark_environment_with_acceleration(), (0.0,)
        )[0]
    ...
    def test_gps_fixes_no_fix_and_exhaustion_do_not_block_monitoring(self) -> None:
        sensor_source = SequenceSensorSource(
            tuple(reading(temperature_c=22.0) for _ in range(4))
        )
        service = MonitoringService(
            sensor_source,
    ...
    def test_new_session_resets_sources_filters_and_prolonged_state(self) -> None:
        source = SequenceSensorSource(
            (reading(temperature_c=27.1), reading(temperature_c=30.0))
        )
    ...
    def test_temperature_humidity_and_light_violations_activate_transport_alarm(
        self,
    ) -> None:
        for values in (
            {"temperature_c": 17.9},
            ...
            with self.subTest(values=values):
                ...
                service = MonitoringService(
                    SequenceSensorSource((reading(**values),)), alarm_output=alarm
                )
                service.start()
                service.step(0.0)
                self.assertEqual(
                    alarm.calls,
                    [
                        (AlarmSource.TRANSPORT_MONITORING, False),
                        (AlarmSource.TRANSPORT_MONITORING, True),
                    ],
                )

    def test_excessive_but_not_moderate_gravity_deviation_activates_transport_alarm(
        self,
    ) -> None:
        for value, expected in ((20.0, True), (15.0, False)):
            with self.subTest(value=value):
                alarm = RecordingAlarmOutput()
                service = MonitoringService(
                    SequenceSensorSource((reading(gravity_deviation_g=value),)),
                    alarm_output=alarm,
                )
                service.start()
                service.step(0.0)
                self.assertEqual(
                    alarm.calls[-1], (AlarmSource.TRANSPORT_MONITORING, expected)
                )
    ...
    def test_start_and_stop_clear_only_transport_alarm_source(self) -> None:
        alarm = RecordingAlarmOutput()
        service = MonitoringService(
            SequenceSensorSource((reading(temperature_c=22.0),)), alarm_output=alarm
        )
