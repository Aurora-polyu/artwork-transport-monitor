...
def workflow_for(
    ...
    return ArtworkWorkflow(
        camera_source=SequenceCameraSource(CameraFrame(f"frame-{number}") for number in range(frame_count)),
...
class ArtworkWorkflowTests(unittest.TestCase):
    def test_first_accepted_detection_toggles_once_per_session(self) -> None:
        workflow = workflow_for(10, (InferenceResult(0, 0.99), InferenceResult(0, 0.99)))
        ...
        self.assertEqual(workflow.states()[0].time_in, datetime(2026, 8, 9, 10, 30, tzinfo=HONG_KONG))

    def test_new_session_allows_the_artwork_to_toggle_again(self) -> None:
        workflow = workflow_for(10, (InferenceResult(1, 0.99), InferenceResult(1, 0.99)))
        ...
        self.assertEqual(workflow.states()[1].time_out, datetime(2026, 8, 9, 10, 30, tzinfo=HONG_KONG))
    ...
    def test_unknown_none_and_inactive_detection_do_not_change_artwork_state(self) -> None:
        workflow = workflow_for(
            25,
            (None, InferenceResult(2, 1.0), InferenceResult(99, 1.0), InferenceResult(0, 0.94)),
        )
        ...
        self.assertTrue(all(state.status is ArtworkStatus.OUT for state in workflow.states().values()))
