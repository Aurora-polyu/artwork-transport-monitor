...
class TransportSessionWorkflowTests(unittest.TestCase):
    def test_complete_normal_session_persists_exports_and_reports(self) -> None:
        with self._workflow(
            (
                reading(
                    0, temperature_c=22.0, humidity_percent_rh=50.0, light_lux=500.0
                ),
                reading(
                    2, temperature_c=22.2, humidity_percent_rh=50.5, light_lux=520.0
                ),
            )
        ) as context:
            ...
            self.assertEqual(
                outcome.report,
                SessionReportGenerator().generate(outcome.stored_session),
            )
            self.assertEqual(
                render_markdown(outcome.report), render_markdown(outcome.report)
            )

    def test_abnormal_session_preserves_immediate_and_one_shot_prolonged_notifications(
        self,
    ) -> None:
    ...
    def test_simultaneous_conditions_and_missing_data_flow_end_to_end(self) -> None:
        ...
        with self._workflow(readings) as context:
            ...
            self.assertEqual(
                [message.condition for message in dispatcher.messages],
    ...
    def test_gps_dropout_is_preserved_through_persistence_and_report_reload(
        self,
    ) -> None:
        readings = tuple(reading(offset, temperature_c=22.0) for offset in (0, 1, 2))
        with self._workflow(
            readings, gps_source=SequenceGPSFixSource(scenarios.gps_dropout().fixes)
        ) as context:
            ...
            self.assertEqual(
                (
                    outcome.report.gps.available_fix_count,
                    outcome.report.gps.no_fix_count,
                    outcome.report.gps.missing_fix_count,
                ),
                (2, 1, 0),
            )
    ...
    def test_two_consecutive_sessions_do_not_leak_state(self) -> None:
        ...
        with self._workflow(readings) as context:
            ...
            second = self._run_and_complete(
                workflow, (0.0,), ended_at=START + timedelta(minutes=1, seconds=8)
            )
