...
def reading(offset_seconds: int, **values: float | None) -> SensorReading:
    return SensorReading(
        timestamp=UTC_START + timedelta(seconds=offset_seconds), **values
    )
...
class SessionReportTests(unittest.TestCase):
    def test_normal_report_is_generated_after_sqlite_reload(self) -> None:
            ...
            self._persist(
                ...
                (
                    ...
                    reading(
                        ...
                        light_lux=520.0,
                        gravity_deviation_g=0.1,
                    ),
                ),
                (0.0, 2.0),
            )
            report = SessionReportGenerator().generate_from_repository(
                repository, "normal"
            )
        ...
        self.assertEqual(
            (report.temperature.minimum, report.temperature.maximum), (22.0, 22.02)
        )
    ...
    def test_abnormal_report_keeps_immediate_and_prolonged_violations_separate(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            repository = SQLiteTransportSessionRepository(
        ...
        self.assertTrue(
            all(
                violation.occurred_at.endswith("+08:00")
                for violation in report.prolonged_violations
            )
        )
    ...
    def test_gps_summary_distinguishes_available_no_fix_and_missing(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self._persist(
                repository, "no-gps", (reading(0, temperature_c=22.0),), (0.0,)
            )
        ...
        self.assertEqual(
            (
                dropout.gps.available_fix_count,
                dropout.gps.no_fix_count,
                dropout.gps.missing_fix_count,
            ),
            (2, 1, 0),
        )
        ...
        self.assertEqual(
            (
                no_gps.gps.available_fix_count,
                no_gps.gps.no_fix_count,
                no_gps.gps.missing_fix_count,
            ),
            (0, 0, 1),
        )
    ...
    def test_repeated_generation_is_deterministic_and_session_isolated(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self._persist(
                repository, "first", (reading(0, temperature_c=27.1),), (0.0,)
            )
            self._persist(
                repository,
                "second",
                (reading(0, temperature_c=22.0), reading(2, temperature_c=22.2)),
                (0.0, 2.0),
            )
            generator = SessionReportGenerator()
        ...
        self.assertEqual(
            first_once.immediate_violations[0].condition,
            Condition.TEMPERATURE_HIGH.value,
        )
